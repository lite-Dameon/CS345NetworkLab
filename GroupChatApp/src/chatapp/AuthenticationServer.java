...
public class AuthenticationServer extends javax.swing.JFrame {
                        ...
                        try{
                            ...
                            if(!userData.equals("")){
                                if(userData.split(":")[0].equals("Auth")){
                                    ...
                                    ArrayList<Integer> friends = groupMembers.get(Integer.parseInt(sp[1]));

                                    String send = "";
                                    for(int name : friends){
                                    ...
                                    //remove from group if it is not leader
                                    // can only leave if all members have left chat
                                    onlineusers.remove((userData.split(":")[1]));
                                    onlineusersPort.remove(userData.split(":")[1]);
                                }
                                else if(userData.split(":")[0].equals("RequestMembersDetail")){
                                    System.out.println("Requesting Members Details ");
                                    int gid = Integer.parseInt(userData.split(":")[1]);
                                    String membersList = createGroupList(gid);
                                    dout.writeUTF(membersList);
                                }
                                sock.close();
                            }
                        } 
    ...
    void refreshList(String name){
        try {
            ...
            String str = "";
           for(int arr : groupMembers.get(Integer.parseInt(name))){
               str += arr+":";
           }
        ...
        if(passwd != null){
            ...
            try {
                ...
                if(passwd.equals(data[2])){
                     System.out.println("User Valid");      
                     if(groupList.get(Integer.parseInt(data[5])) == null){
                         System.out.println("Master contacted for group no " + data[5]);
                         groupList.put(Integer.parseInt(data[5]),new ArrayList<>());
                         // first user in the list is always leader
                         groupList.get(Integer.parseInt(data[5])).add(data[3]+":"+data[4]);
                         System.out.println("Data 5 supposed to be group no "+ data[5] +  " and corresponfing list is " + groupList.get(Integer.parseInt(data[5])).toString());
                         master = "YES";
                         groupMembers.put(Integer.parseInt(data[5]),new ArrayList<>());
                         groupMembers.get(Integer.parseInt(data[5])).add(Integer.parseInt(data[1]));
                     }
                     else{
                         groupList.get(Integer.parseInt(data[5])).add(data[3]+":"+data[4]);
                         System.out.println("other clients contacted for group no " + data[5]);
                         String membersList = createGroupList(Integer.parseInt(data[5]));
                         //send new list to master of group data[5]
                         int gid = Integer.parseInt(data[5]);
                         groupMembers.get(Integer.parseInt(data[5])).add(Integer.parseInt(data[1]));
                         sendListToMaster(gid,membersList);
                     }
                    dout.writeUTF("Valid:"+master);
                     for(String s: data){
                    ...
                    dout.writeUTF("InValid:N"+master);
                }
            } catch (IOException ex) {
                Logger.getLogger(AuthenticationServer.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void sendListToMaster(int gid, String list) {
        try {
            String master = groupList.get(gid).get(0);
            String ipAddress = master.split(":")[0];
            int port = Integer.parseInt(master.split(":")[1]);
            
            Socket s = new Socket(ipAddress,port);
            DataOutputStream out = new DataOutputStream(s.getOutputStream());
            out.writeUTF("MembersList:"+list);
            s.close();
        } catch (IOException ex) {
            Logger.getLogger(AuthenticationServer.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    String createGroupList(int id){
        System.out.println("Id of new logined user is " + id);
        
        ArrayList<String> list = groupList.get(id);   
        if(groupList.get(id) == null){
            System.out.println("Group lidt is null");
            System.err.println("List is null " + "trying seeing data : " + groupList.entrySet());
            return "";
        }
        else
        System.out.println("List is Not null " + groupList.entrySet());
        ...
        System.out.println("New group is : " + group);
        return group;
    ...
    HashMap<Integer,ArrayList<String> > groupList;
    HashMap<Integer,ArrayList<Integer> > groupMembers;
    Socket sock;
